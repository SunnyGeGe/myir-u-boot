...
int power_init_board(void)
{
	...
	/* 
	 * LS2 supply 5v for usb0 vbus, which is work in peripheral mode.
	 * set current limit to 500 mA, turn off by default.
	 */
	int ret = 0;
	ret = tps65218_reg_write(TPS65218_PROTECT_L1, TPS65218_CONFIG2,
		0x02, TPS65218_CONFIG2_LS2ILIM_MASK);	/* 500 mA */
	if (ret) {
		printf("%s: Set LS2 current limit to 500 mA failed!\n");
		return ret;
	}
		
	ret = tps65218_reg_write(TPS65218_PROTECT_L1, TPS65218_ENABLE2,
		0, TPS65218_ENABLE2_LS2_EN); 		/* turn off */
	if (ret) {
		printf("%s: Turn off LS2(vbus0) failed!\n");
		return ret;
	}
	
	printf("Init vbus0: 500mA@5V, OFF\n");
	
	/* 
	 * LS3 supply 5v for usb1 vbus, which is work in host mode.
	 * set current limit to 500 mA, turn on by default.
	 */
	ret = tps65218_reg_write(TPS65218_PROTECT_L1,TPS65218_CONFIG2,
		0x02<<2, TPS65218_CONFIG2_LS3ILIM_MASK);	/* 500 mA */
	if (ret) {
	...
	ret = tps65218_reg_write(TPS65218_PROTECT_L1,TPS65218_ENABLE2,
		TPS65218_ENABLE2_LS3_EN, TPS65218_ENABLE2_LS3_EN); /* turn on */
